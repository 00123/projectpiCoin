...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("pi")
    updateDisplayUnit();
}
...
{
    QString ret = QString("piCoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non pi unit input in lnReqAmount, we generate the URI with pi as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::pi, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
